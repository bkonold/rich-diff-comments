# Microsoft Edge Add-ons submission — v1.9.0
...
v1.9.0 is a navigation-sync polish release building on 1.8.0's file-scoped counter and file-tree click sync. GitHub's own left-side file tree now visibly highlights the file you're reading (blue rail + tint on the matching row) as you scroll or click through the PR. The highlight updates instantly on clicks (Outline sections, thread cards, change cards) instead of waiting for the smooth-scroll to finish. Clicks anywhere on a tree row — filename or padding — now count, matching GitHub's own click zone. No new permissions, no new host permissions, no manifest changes other than the version bump.
...
- **Zip:** `rdc-1.9.0.zip` (in this folder)
...
Includes all changes from v1.9.0, v1.8.0, and v1.7.0.
