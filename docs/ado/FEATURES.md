...
- Comment actions differ in scope. Edit and Delete apply only to the reviewer's own comments. Azure DevOps' native Copy link action identifies the conversation thread rather than an individual reply, so extension parity should copy the same thread-level destination and explain that granularity through its label or feedback. The native surface also exposes Like, Resolve/Reopen, and a work-item picker with Issue, Epic, and Task choices. Work-item creation should open the normal ADO form and link the conversation instead of creating an item silently. Capture the native link and creation requests before implementation rather than assuming URL or REST shapes.
...
- The sidebar header's × fully hides the panel. A small launcher restores its saved position and size and opens the Threads tab. This behavior is under UX review because a fully hidden sidebar may be difficult for first-time users to rediscover; do not use it as the GitHub parity target until that review is complete.
