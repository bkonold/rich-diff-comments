...
- The sidebar can collapse to its header and can be toggled with the keyboard, but unlike ADO it has no × control that fully hides it and no compact launcher for restoring it. On the React Files-changed view the collapsed header docks into GitHub's sticky files toolbar instead of floating; it stays visible there, so this is still collapse rather than dismissal.
