...
1. **Can't find the collapsed bar.** Once collapsed, the slim header was the only visible thing — and the previous pale-blue tint blended into GitHub's own surface colors. 1.0.3 switched the header to solid `--fgColor-accent` (GitHub's link blue) so the bar stood out. Now discoverability comes from placement instead: on the React Files-changed view the collapsed bar docks into GitHub's own toolbar (see [Collapsed strip docks into GitHub's files toolbar](#collapsed-strip-docks-into-githubs-files-toolbar)), and the expanded header went back to a light accent tint (`color-mix` of `--bgColor-accent-muted` with 8% `--fgColor-accent`, muted icons) that matches the docked pill.
...
3. **No way to call it back.** Bound two global keydowns (skipped when typing in inputs / `Ctrl|Meta|Alt` held):
   - `t` → `toggleSidebarCollapsed()` — flips the collapsed class. Collapsed state is not persisted: every page load creates the shell collapsed, and rebuilds reuse the shell so an expand lasts until the next load.
   - `Shift+T` → `resetSidebarLayout()` — clears `SIDEBAR_POS_KEY` / `_SIZE_KEY`, drops inline styles, rebuilds, and expands. Recovers from any offscreen / lost-panel state without DevTools.
...
Two entry points share one scroll-and-click sweep (`flipAllMdDiffs(target)`):

- **Header button / `b`** (`toggleAllMdRendering`) is a page-level rich ↔ source toggle for every Markdown file. It shows a book icon while any Markdown file is still in source view and a code icon once all are rendered, and flips them all back to source from there.
- **Empty-state CTAs** in the Threads and Changes panes (`expandAndRenderAllMd`) only render (never flip back) and expand the sidebar first.
...
- **Lazy-render workaround.** GitHub mounts file headers only as they enter the viewport (IntersectionObserver). A single top-of-page scan finds at most ~2 toggles. `flipAllMdDiffs` scrolls the page top-to-bottom in `0.8 × innerHeight` steps with a 100 ms dwell, scanning after each step. Behind a translucent `.grdc-render-overlay` ("Loading Markdown files…") so the user doesn't see the page jumping. Early-exits when `seen.size >= expectedMd` (count from `routeData.diffSummaries`), restores the user's original scroll, then calls `buildThreadsSidebar()` after a 400 ms delay so newly-rendered threads populate. The empty-state CTAs expand the sidebar if it was collapsed — without that, users mistakenly thought "only some files opened" because the threads pane was hidden behind the slim bar. The header button / `b` deliberately does not expand the sidebar or switch tabs: it is a view toggle for the page, and the result is read in the diff itself.

### Collapsed strip docks into GitHub's files toolbar

On the React `/changes` view, collapsing the sidebar moves the whole `.grdc-sidebar` element into GitHub's sticky files toolbar; expanding moves it back to `<body>`. `setSidebarCollapsed()` is the single entry point for the collapsed class and calls `syncSidebarToolbarDock()`.

- **Captured DOM (github.com, 2026-09):** the toolbar is `section[class*="PullRequestFilesToolbar-module__toolbar"]` (`position: sticky; top: 0; z-index: 11`, 60px tall). Its right-hand group starts with `div[class*="PullRequestFilesToolbar-module__file-controls__"]`, the "N / M viewed" progress; we insert `.grdc-toolbar-dock` immediately before it. GitHub's neighbouring icon buttons are Primer `IconButton`s with `data-variant="invisible" data-size="small"`: 28×28, `--fgColor-muted`, 6px radius. The docked CSS mirrors that.
- **Why move the whole element, not just the header:** dozens of call sites do `sidebar.querySelector('.grdc-sidebar-count')` etc. and `document.querySelector('.grdc-sidebar')`. Moving the element keeps every lookup valid.
- **Inline styles vs. `!important`:** the persisted drag position and size live in the sidebar's inline styles. Docked CSS overrides them with `!important`, so nothing needs clearing; they apply again the moment the sidebar returns to `<body>`. `container-type` must be reset to `normal` while docked, or inline-size containment collapses the auto-width element to zero.
- **React re-renders:** GitHub can throw away the toolbar subtree, taking the docked sidebar with it. The body MutationObserver schedules one `syncSidebarToolbarDock` per animation frame. `sidebarRef()` keeps a reference to a *docked* sidebar so it can be re-docked into the new toolbar, or returned to `<body>`. Intentional removals go through `removeSidebar()`, which drops that reference so a sidebar we removed on purpose is never resurrected.
- **Fallback:** when the selectors don't match (classic `/files`, GitHub redesign) nothing is docked and the collapsed strip floats as before.
...
| Comment on a top-level `<li>` (with no nested list) rendered AFTER the entire parent `<ul>`/`<ol>` (1.0.2) | The 1.0.1 nested-list fix only branched when the `<li>` had a direct-child `<ul>`/`<ol>`. Leaf `<li>`s fell through to `topUnderlinedAncestor(element) || element`. When GitHub's rich-diff wrapped the whole list in `<ins>` or `.added` / `.removed` (common when a list is added or deleted in the PR), the underline-escape walked up past the parent `<ul>` / `<ol>` and returned the list itself, so `.after(thread)` placed the thread *after* the closing `</ul>` — under the last bullet instead of under the targeted one. Only visible on lists that lived inside an inserted/deleted block, which is why it slipped past 1.0.1 testing (the nested-list repro was inside a non-`<ins>` list). | Extended `siblingAnchor`'s `LI` branch to also handle the leaf case: when there is no nested list, return a proxy whose `.after(node)` appends *inside* the `<li>` itself (`element.appendChild(node)`), with `nextElementSibling` exposed for the chaining logic. We deliberately skip the underline-ancestor escape for `<li>` — the whole point is to keep the insertion inside the `<li>` rather than escape past the parent list. In practice GitHub's prose-diff wraps each `<li>`'s text in its own `<ins>` (not the whole list), so the injected box doesn't visibly inherit the underline anyway. |\n| Edit own comments (1.0.1) | The `⋯` menu / edit affordance for review comments lives in source-diff view only; rich-diff users had to leave the page to fix typos in their own comments. | Captured endpoint via DevTools: `PUT /pull/<n>/page_data/update_review_comment?body_version=<sha256>` with body `{"body":"new text","commentId":"<dbId-as-string>"}`. **Two payload-shape gotchas:** (1) the field is `body` (not `text` like create_review_comment). (2) `commentId` must be a string. **`body_version`** is GitHub's per-comment hash, used as a conflict token. The 200 response includes a fresh `bodyVersion` for the updated body — we cache it on the local comment record so a second edit in the same session uses the new hash. The initial `/changes` fetch *may* include `bodyVersion` per comment; if so we use it. Otherwise we fall back to `sha256Hex(originalBody)` which matches in practice. UI: `⋯` menu on each comment whose author matches the viewer login (read from the `dotcom_user` cookie). Click Edit → inline editor with original body → Save invokes the endpoint and replaces the rendered body with the server's returned HTML. |
...
| Rendered Table of Contents navigation jumped to the top on the first click or did nothing on repeated clicks (1.10.0) | Rich diff removes heading ids. Native fragment navigation could therefore jump to the top and race the extension's first manual scroll; browsers also emit no `hashchange` when the selected fragment is already current. | Added a capture-phase delegate for same-page links inside rich diff. It resolves the heading within the clicked file, adds changed fragments with `history.pushState()` to avoid native anchor scrolling, and scrolls every click directly. Browser regressions cover first and repeated clicks, different destinations, and Back navigation. |
| Sidebar scrollbar thumb slid toward the bottom while its bottom-right resize handle was dragged (1.10.0) | Width changes can trigger scroll anchoring, and Chromium can treat the overlapping bottom-right scrollbar area as a held track interaction whose auto-repeat continues after the pointer leaves the corner. Both alter a pane's `scrollTop` during the native CSS resize gesture. | Detect resize-handle gestures and lock the Changes, Threads, and Outline scroll positions until mouseup. Restore on both resize and scroll events, then reliably release through captured mouse/pointer-up or window blur while continuing to persist the final dimensions. |
| “Render all Markdown files as rich-diff” showed progress on a large PR, then all rendered files disappeared (1.10.0) | GitHub's `/pull/<n>/changes?mode=virtualization#diff-…` “optimized for large pull requests” surface virtualizes the file list. Scrolling mounts files long enough for their rich-diff toggles to work, but GitHub later unmounts those nodes and discards their local rendered state. The native `Switch to single file mode` link uses `/pull/<n>/changes?mode=single#diff-…`, but automatic mode navigation was rejected as surprising. | Detect `mode=virtualization` in the current URL, with the visible `mode=single` transition link as a fallback signal. Hide the header and empty-state bulk-render controls, guard the bulk function itself, and explain that Markdown files must be reviewed and switched to rich diff one at a time. |
| Threads offered “Render all Markdown files as rich-diff” when the visible Markdown was already rendered and simply had no comments (1.11.0) | The empty state used thread count as a proxy for rich-diff state. A rendered file with zero threads therefore looked identical to a fresh source-only review surface. | Track rendered Markdown paths from live rich-diff roots and successful toggle scans, independently of comments. Compare those paths with Markdown files in the changes payload; show bulk actions only while an eligible file remains unrendered. Preserve observations for GitHub's lazy-unmounted files, update mounted files in both directions, and retain the virtualized-large-PR guidance as the higher-priority state. |
| Sidebar disappeared after opening a PR from the repository Pull requests list (1.10.0) | A full load of `/<owner>/<repo>/pulls` created a document outside the content script's match. GitHub then used client-side navigation into `/pull/<n>/changes`, so Chrome did not inject a new extension runtime. Direct PR-tab navigation worked because the broader `/pull/*` runtime was already present. | Added the Pull requests list to the content-script match while keeping the runtime visually inactive there. The existing URL poll initializes on entry to Files changed. A PR-identity transition now clears route data, comments, source maps, raw-source and mention caches; generation guards prevent slower requests from the previous PR from repopulating cleared state. Browser coverage starts on `/pulls`, performs an in-document transition, and verifies the sidebar and Outline initialize. |
