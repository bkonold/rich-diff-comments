...
| **YAML frontmatter** | GitHub renders a leading `---\n...\n---\n` block as a 2-column `<table>` at the very top of the rich-diff. Two failure modes: (a) the matcher tries to text-match into frontmatter SOURCE on behalf of a body block whose text happens to overlap a key/value; (b) more commonly, a rendered cell's text content (e.g. a `note:` value mentioning a library name) substring-matches body content downstream — a Related Features table, a Change Log row — so `lastOffset` jumps to the end of the file. Every block after frontmatter then fails the `result.offset > lastOffset` check and falls back to the `lastLine + 1` nudge, anchoring at the last lines of the file. | Two complementary defenses + a feature: (a) `findFrontmatterRange()` detects the leading `---`/`---` block and `buildSourceIndex` masks those lines so frontmatter text is unreachable on the *source* side; (b) `buildLineMap` resolves the rendered frontmatter `<table>` (always the first table in the rich-diff when frontmatter is present) and skips inner / nested blocks on the *DOM* side so their text can't poison `lastOffset`. **Additionally**, each direct `<tbody> > <tr>` of that table is mapped to its corresponding top-level YAML key's source line via `frontmatter.keyLines`, so reviewers can leave inline `+` comments on `area:`, `status:`, `related:`, etc. — the metadata still gets a review surface, just one driven by structural mapping rather than text matching. |
