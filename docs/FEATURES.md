...
- [ ] **P0 — Azure DevOps Files-page sidebar and one-click Preview (v1.0.1).** Show the ADO sidebar immediately on the PR Files landing page instead of waiting for the user to locate a Markdown file, select Preview, download its source, and finish line mapping. When no rendered Markdown is active, show an **Open Markdown Preview** action that opens the selected Markdown file—or the first changed Markdown file—and selects ADO Preview. First activation must wait for a late TreeEx leaf/List dispatcher and update native selection before restoring Preview, so bare and non-Markdown Files routes do not require a manual Markdown click. Preview-option discovery must remain inside ADO's view-mode popup so changed files whose names end in `preview` cannot hijack setup navigation. Load file/thread inventory independently, prioritize the active file, show progressive PR-wide analysis, isolate failed files, and expose local startup timing through the diagnostic probe. Full design and acceptance criteria: [ADO_ADAPTER_PLAN.md → Iteration R](./ADO_ADAPTER_PLAN.md#710-iteration-r--immediate-files-page-sidebar-and-one-click-preview-v101).
- [ ] **P0 — Azure DevOps reliable nested-file navigation (v1.0.1).** Prevent unrelated rows and duplicate basenames from qualifying as file-tree targets, expand exact collapsed ancestors, and update ADO's native selected-file model before Preview restoration. Distinguish cold-start TreeEx readiness from navigation after Preview is established; keep an exact same-PR route only as a final fallback when the tree cannot expose the requested file. Full design and acceptance criteria: [ADO_ADAPTER_PLAN.md → Iteration Q](./ADO_ADAPTER_PLAN.md#79-iteration-q--reliable-nestedvirtualized-file-navigation-v101).
- [ ] **P0 — Preserve Azure DevOps sidebar state across file navigation (v1.0.1).** Cross-file Changes, Threads, and Outline navigation should remap only the newly opened Preview—not reload PR-wide catalogs, flash first-run setup, or reset to the first change. Materialize offscreen native tree rows before the last-resort full-page fallback. Match the GitHub extension by listing only Markdown-file review threads; non-Markdown threads have no rendered review destination and must never create a pending Preview jump. Full design and acceptance criteria: [ADO_ADAPTER_PLAN.md → Iteration S](./ADO_ADAPTER_PLAN.md#711-iteration-s--preserve-pr-wide-state-during-sidebar-navigation-v101).
