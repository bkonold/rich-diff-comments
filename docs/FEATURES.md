...
- [x] **288 tests** in two suites — fast unit tests + slower e2e:
  - **268 Node:test unit tests** (`npm test`, ~2s) covering pure helpers (line matching, response parsing, markersMap parsing, table arithmetic, markdown preview, HTML escaping, time formatting, sidebar helpers incl. `isMarkdownPath`, heading slugify, outline tree + thread attribution + fold-at-level), the per-file block→line mapping in [src/lib/lineMap.js](../src/lib/lineMap.js) against jsdom fixtures (regression net for YAML frontmatter, mermaid, deleted blocks, and table-row arithmetic), `styles.css` coverage (every host tag `buttonAnchor()` can target must have a positioning override so the `+` is visible), and `+`-button attachment (the `<th>`-host case for YAML frontmatter rows that broke in v1.5.1).
  - **20 Playwright e2e tests** (`npm run test:e2e`, ~35s) running in real headless Chromium against captured rich-diff HTML fixtures — no live github.com. Covers `+` button visibility & click behavior (including the v1.5.1 `<th>` regression), the full extension init → mapping → button-attachment pipeline, comment-box mounting, and keyboard shortcuts (`t`, `1`/`2`/`3`). Catches the class of bug that passes unit tests but fails in a real browser (computed CSS, real `:hover`, real keydown events).
  - `npm run test:all` runs both suites. jsdom and Playwright are devDependencies only — the shipped extension zip has zero npm runtime deps.
...
- [ ] **P2 — Extend Changes navigation to non-Markdown files** — today the Changes tab and the `[` / `]` shortcuts only walk blocks inside `.prose-diff` rich-diff containers, which means they cover `.md` / `.mdx` / `.rst` etc. and nothing else. A PR that also touches `.py` / `.js` / `.yaml` / `.cs` files has those changes invisible to our navigation, so a reviewer can't click through *all* the changes in one pass.

  **Reviewer mental model this addresses** (from 2026-06 user feedback):
  - **Sequential review** — small change, or familiar codebase: the reviewer clicks `]` through every change in the PR in DOM order. Today they have to switch tools (or scroll manually) once they leave a Markdown file.
  - **Targeted review** — large or unfamiliar change: the reviewer jumps to specific files via the GitHub file tree, then reads inside each one. The sidebar's Outline pane already supports this for `.md` files; extending Changes to non-MD files would let the same workflow span the whole PR.

  **Scope:**
  - Collect change blocks from GitHub's native **source-diff** containers (`.js-file-content` / `.diff-table` rows), in addition to the existing `.prose-diff` walk. Source-diff already labels each line as added / removed / unchanged via row class, so identifying "change blocks" is straightforward — group contiguous changed rows in the same file as one stop (similar to how source-diff itself groups hunks).
  - One card per source-diff hunk (not per line) to avoid flooding for refactors. Card shows: kind glyph, file path, hunk line range, first changed line as snippet.
  - Whole-new and whole-deleted non-MD files: emit the same "NEW FILE" / "DELETED" summary card used for `.md` files (1.7.0 behaviour), keyed off the same `changeType` from `routeData.diffSummaries` — we already fetch this for every file in the PR.
  - Click → scroll to the hunk's first changed line. Same flash highlight as today.
  - Counter format stays `current/total` across all files (MD + non-MD interleaved in DOM order).
  - Outline tab stays MD-only — non-MD files don't have a heading tree to outline.

  **Trade-offs to think about before implementing:**
  - **Card cardinality**: a PR that touches 200 lines across 30 hunks would produce 30 cards. Acceptable if grouped per-hunk; problematic if per-line. The Markdown side already groups per reading-unit (one card per paragraph), so consistency with that level of granularity is fine.
  - **Source-diff is collapsed by default on large files**: GitHub auto-collapses long source-diff files behind a "Load diff" button. If we walk only visible source-diff containers we'll miss collapsed files; if we eagerly expand them we slow the page down. MVP: walk only what's visible; document the limitation; offer a future "render all source-diff" sibling to the existing "render all .md as rich-diff" button.
  - **No anchor for "Open all in source-diff"** equivalent. The Markdown side has the book button; the source-diff side has no equivalent batch-render today.

  **MVP cut**: cover sequential review (the more common case) by adding source-diff change blocks to the existing Changes pane, keyed by `changeType` for ADDED/REMOVED summary cards. Defer the batch-expand-collapsed-files question to a follow-up.
- [x] **~~P1~~ — GitHub file-tree click sync** (shipped in 1.8.0) — clicking a file in GitHub's own left-side file tree (or the "Jump to file" dropdown, or the file-list overview, or any in-page anchor pointing at a diff container) now jumps our sidebar's Changes & Threads counters to that file's first item. Previously the sidebar only followed indirectly via the scroll-follow IntersectionObserver, which had to wait for a heading to become topmost — delayed on all files, and never firing at all for files with no headings (a code-only `.md`, a whole-file NEW FILE card).

  **Implementation:** a single capture-phase document click delegate installed once in `observe()` catches any `a[href*="#diff-"]` click; resolves the hash to a `div[id="diff-<hash>"]` container; extracts the path via the shared `getFilePath()` helper; calls `jumpSidebarListsToFile(sidebar, path)` before GitHub's scroll animation starts. No selector coupling to any specific tree component — the delegate targets the anchor pattern (`#diff-<hash>`) that every GitHub navigation surface produces, so it survives future GitHub file-tree redesigns.

  **Other sync gaps captured during the design pass but deferred:**
  - **GitHub's own file-tree active-state indicator** — closed in Unreleased (2026-07): we add our own `grdc-file-tree-active` class to the tree row / dropdown row / list-overview row that matches the currently-viewed file. Purely visual (rail + background tint via CSS), no synthetic click that would double-fire GitHub's own smooth-scroll. Coexists with whatever GitHub renders as its own "you clicked this" state. Applies at the anchor level rather than the row wrapper because different tree UIs use different ancestor structures.
  - **Outline pane active-heading vs. Changes / Threads counters** can disagree at file boundaries during a smooth-scroll for one animation frame. Small enough that no one has actually reported it; leaving alone.
- [x] **~~P1~~ — Header change / thread icons jump to the current file's first item** (shipped in 1.8.0) — the page icon and speech-bubble icon in the sidebar header used to be global "next change" / "next thread" shortcuts (introduced in 1.6.0). On a multi-file PR the semantics felt wrong: the reviewer's mental model is "I'm in file X, take me to the first item in file X", not "advance the global counter by one". Now: single tap → jump to the first change / thread in whichever file's rich-diff is currently topmost in the viewport (uses a shared `getCurrentViewFilePath()` helper). If the current file has zero items in that list (or no `.prose-diff` intersects the viewport at all), the icon falls back to the pre-1.8.0 global-advance behaviour so it's never a dead click. Tooltips updated to match ("First change in this file — or next change globally if the file has none"). The `[` / `]` and `j` / `k` keyboard shortcuts kept their global-walk semantics — nothing was taken away; the icon just got a more useful default action. **From 2026-06 user feedback:** *"If we click the change / threads icon → goes to the first change / thread of our current viewing file."*
- [x] **~~P2~~ — Hide the sidebar when the user leaves the Files-changed tab** (shipped in 1.8.0) — the sidebar used to stay visible on the Conversation / Commits / Checks tabs (and on non-PR pages reached via GitHub's SPA navigation) because its visibility check keyed off "is this a PR page?" rather than "is this the Files-changed tab?". After the 1.7.0 default-position move to top-centre this became especially visible — the sidebar would sit right on top of the PR title strip on Conversation. Now: the SPA-navigation poller (`maybeInit`) explicitly removes the sidebar and clears injected `+` buttons whenever `parsePRUrl()` returns null (i.e. the URL is not `/pull/<n>/files` or `/pull/<n>/changes`). Coming back to Files-changed re-runs `init()` on the next poll cycle (≤ 400 ms), which rebuilds the sidebar from the saved position / size / collapsed state in `localStorage` — so nothing is lost, the sidebar just gets out of the way when it has nothing to do. **From 2026-06 user feedback:** *"Why we click from Files changed to Conversation, our sidebar doesn't disappear?"*
- [ ] **P1 — "Current file only" filter for Changes and Threads panes** — the 1.8.0 file-scoped counter (`3/8 (45)`) already tells the reviewer "3 of 8 in this file, 45 in the PR", but the pane still shows every card from every file. For a large PR (say 45 changes across 8 files, 12 threads across 5 files) the reviewer often wants to **focus on just the file they're reading** — hide everyone else's cards so `j` / `k` and `[` / `]` walk only the current file's items.

  **From 2026-07 user feedback:** *"Can we add a filter to Changes and Threads that we can filter the Changes / Threads of current file only?"*

  **Design:**
  - **Separate toggles per pane, not shared** — Threads and Changes serve different purposes. A common workflow is skim threads globally (see the whole conversation), but review changes file-by-file. One shared toggle would force the two to move together and lose that flexibility.
  - **Follows scroll automatically** — when the filter is on, "current file" tracks the viewport (same `getCurrentViewFilePath()` helper the counter uses). Manual "stuck on the file you were on when toggled" behaviour would drift out of sync as the reviewer scrolls. Auto-follow keeps the filter honest.
  - **Independent of the existing "Unresolved only" filter on Threads** — AND semantics: `filter="current file only" AND "unresolved only"` = "unresolved threads in this file". Both toggles cooperate; neither pre-empts the other.
  - **Two surfaces per pane for discoverability:**
    - **Header icon** (small file-with-focus icon) next to each nav cluster — works in collapsed mode too, mirrors the existing "unresolved" funnel icon pattern.
    - **In-pane checkbox** at the top of the list — discoverable when the user is already looking at the pane.
    - Toggling either updates the other (single source of truth in `localStorage`).
  - **Counter format changes when filter is on:**
    - Without filter: `3/8 (45)` — file-scoped position + PR total hint.
    - With filter on: `3/8` — the `(45)` tail becomes redundant since the visible list IS filtered to the file.
  - **Empty state when current file has 0 items in the filtered pane:**
    - "No threads in this file · **Show all**" (button toggles the filter off) — same for Changes.
    - Prevents dead-end confusion where the pane is empty and the user doesn't know why.
  - **Keyboard nav (`j` / `k` / `[` / `]`) walks the visible list** — automatic since our nav already walks rendered cards, not the underlying full list.
  - **State persists per browser** — `localStorage['grdc_threads_current_file_only']` and `grdc_changes_current_file_only`, both `'1'` / `'0'`. Reviewer's preference sticks across PRs.

  **Implementation sketch:**
  1. Two new `localStorage` keys and two new sidebar state getters (`isThreadsFileOnly()`, `isChangesFileOnly()`).
  2. `buildThreadsSidebar` filters `threadEls` by `getCurrentViewFilePath()` when the flag is on. Same for `buildChangesPane`.
  3. Header icon: add a new `<button class="grdc-sidebar-file-filter">` on both nav clusters, sibling to the existing `.grdc-sidebar-header-filter`. `aria-pressed` toggles just like the unresolved funnel.
  4. In-pane checkbox: add to the existing threads filter row (which already has "Unresolved only"); create a matching row above the Changes list.
  5. Scroll listener already re-fires at file boundaries (1.8.0). When either filter is on, also call `buildThreadsSidebar` / `buildChangesPane` on file crossings so the pane content updates. Debounce with the same rAF pattern to avoid rebuild storms.
  6. `renderScopedCounter` accepts a new `hideTotal` flag; renderers pass `true` when the filter is on so the `(T)` tail drops.
  7. Empty state: extend the existing empty-message rendering with the "Show all" escape hatch.

  **Trade-offs the design accepts:**
  - **Sidebar width doesn't shrink** when the visible list shrinks — that's fine, the sidebar has resize-persistence already and shrinking on toggle would be jarring.
  - **File-boundary scroll rebuilds one pane per boundary crossed** — slightly more work than the counter-only rebuild today, but still cheap (a few dozen DOM nodes) and only fires when the filter is on.
  - **Keyboard nav can't cross files when filter is on** — that's the *point* of the filter, but users switching between "focused" and "global" mid-review will need to toggle back off. Header icon makes this cheap.

  **Not included in this feature** (future work):
  - "Current file only" for Outline — Outline is already file-scoped (each file gets its own section header), and the "current file" filter would just collapse other files' sections, which the fold buttons already do.
  - Multi-file selection (filter to "this file + one other") — YAGNI; the current-file-only case is the ask.

  **⚠️ 2026-07-02 attempt: implemented and reverted.** The full feature was built out — localStorage keys, in-pane checkboxes on both panes, filter logic in `buildThreadsSidebar` / `buildChangesPane`, scroll-listener rebuilds on file crossings, `pendingFileFilterOverride` for click-driven jumps, "Show all files" empty-state escape. It worked in the happy path but **the interaction with scroll-driven follow was fragile** — the pane would frequently show the wrong file's cards or blink `0/0`, especially when scrolling back up or across multiple files quickly. The full v1.9.0 filter code was reverted. Lessons captured below for the next attempt.

  ### Lessons learned (2026-07-02) — for the next attempt

  The feature is worth having, but the interaction between "user is scrolling" and "sidebar is following" was harder than the surface complexity implies. Notes for next time:

  1. **The single source-of-truth for "current file" is fragile.** `getCurrentViewFilePath()` returns `''` in many transient states: mid-scroll between files, sticky-line heuristic misses, GitHub React swaps a container mid-frame. Any filter that keys off it will occasionally see `''` and must decide: filter to nothing (empty pane) or fall through to unfiltered (all files). Both look broken. Neither is right without a persistent "last known good" cache — plain `getCurrentViewFilePath()` isn't enough as a filter key.

  2. **The scroll listener and click handler cannot share the same code path if that path pins state.** `jumpSidebarListsToFile` was designed for user *clicks*: pin the Outline highlight through the smooth-scroll animation so the scroll listener doesn't yank it back mid-flight. When the scroll listener itself called `jumpSidebarListsToFile` on a natural boundary crossing, the pin then suppressed the *next* boundary crossing for 1.5 s — quick scrolls across 3+ files left the pane stuck on the first crossing. The `pin: false` opt-out I added helped but didn't fully solve it — the pane could still lag one crossing behind for reasons I didn't finish debugging. Next attempt: **give the scroll-listener path its own dedicated function that doesn't share pin state with click handlers**, even at the cost of duplicating some code.

  3. **Rebuilding a pane during scroll invalidates `currentIdx` invisibly.** Every `buildChangesPane` / `buildThreadsSidebar` rebuild recomputes the cards array. If the user was on card `#3` and the rebuild filters down to a different file with only 2 cards, `changesCurrentIdx = 3` is now out of range, and the next keyboard nav goes somewhere unexpected. The reset-to-first-in-new-file logic mostly worked but had edge cases (e.g. selected card belongs to the *previous* file, not the newly-viewed one). Cards `data-grdc-path` needs to be the authoritative anchor for restoring position, not the flat array index.

  4. **The 1.8.0 file-scoped counter (`3/8 (45)`) already covers ~80% of the value.** Users get file-scoped position AND PR total in one glance without any filter, and keyboard nav still walks the global list. The filter would let them *hide* other files' cards, which is a stronger form of focus — but the "always know where you are + walk globally" pattern is arguably clearer than "hide everything but this file + walk locally" because it never traps the user in a subset they can't easily escape from.

  5. **Consider the alternative UX**: instead of a filter that *hides* other files' cards, group them under collapsible file headers (like Outline already does). Same "focus on this file" outcome but nothing is hidden — the user can always see what's in other files and click into them. Might be simpler than the toggle+auto-follow+pin/override dance.

  6. **If we retry, isolate the filter completely from the scroll-follow / click-jump plumbing.** Keep the two systems orthogonal:
     - Scroll listener + click handlers → update `currentIdx` and highlights (as they do today, unchanged)
     - Filter → a pure display layer that hides cards via CSS (`display: none`) based on the current viewport file, without touching `_grdcChangeBlocks`, `changesCurrentIdx`, or triggering any pane rebuild
     - This keeps keyboard nav walking the global underlying list; the filter is purely visual clutter reduction
     - Trade-off: `[` / `]` would then skip over hidden cards but the counter position (`3/8`) would still refer to the visible list, requiring a small mapping. Simpler than the "rebuild on every crossing" approach we tried.

  **Files touched in the reverted attempt** (for the next attempt to reference or diff against): `content.js` (~370 lines added — new localStorage keys, `isThreadsFileOnly` / `isChangesFileOnly`, filter logic in both pane builders, `pendingFileFilterOverride`, scroll-listener rebuild hook, extended empty-state), `styles.css` (new `.grdc-sidebar-filter-row` container), CHANGELOG and FEATURES.md entries. See git commit history around 2026-07-02 in unpushed working state (never committed to `main`).
...
- [x] **~~P0~~ — Azure DevOps Files-page sidebar and one-click Preview (shipped in 1.1.0).** Show the ADO sidebar immediately on the PR Files landing page instead of waiting for the user to locate a Markdown file, select Preview, download its source, and finish line mapping. When no rendered Markdown is active, show an **Open Markdown Preview** action that opens the selected Markdown file—or the first changed Markdown file—and selects ADO Preview. First activation updates native selection before restoring Preview, so bare and non-Markdown Files routes do not require a manual Markdown click. Preview-option discovery remains inside ADO's view-mode popup so changed files whose names end in `preview` cannot hijack setup navigation. File/thread inventory loads independently, the active file is prioritized, PR-wide analysis reports progressive results, failed files are isolated, and local startup timing remains available through the diagnostic probe. Full design and acceptance criteria: [ADO_ADAPTER_PLAN.md → Iteration R](./ADO_ADAPTER_PLAN.md#710-iteration-r--immediate-files-page-sidebar-and-one-click-preview-v110).
- [x] **~~P0~~ — Azure DevOps reliable nested-file navigation (shipped in 1.1.0).** Prevent unrelated rows and duplicate basenames from qualifying as file-tree targets, expand exact collapsed ancestors, and update ADO's native selected-file model before Preview restoration. Keep an exact same-PR route only as a final fallback when the tree cannot expose the requested file. Full design and acceptance criteria: [ADO_ADAPTER_PLAN.md → Iteration Q](./ADO_ADAPTER_PLAN.md#79-iteration-q--reliable-nestedvirtualized-file-navigation-v110).
- [x] **~~P0~~ — Preserve Azure DevOps sidebar state across file navigation (shipped in 1.1.0).** Cross-file Changes, Threads, and Outline navigation remaps only the newly opened Preview instead of reloading PR-wide catalogs, flashing first-run setup, or resetting to the first change. Offscreen native tree rows are materialized before the last-resort full-page fallback. Only Markdown-file review threads are listed because non-Markdown threads have no rendered review destination. Full design and acceptance criteria: [ADO_ADAPTER_PLAN.md → Iteration S](./ADO_ADAPTER_PLAN.md#711-iteration-s--preserve-pr-wide-state-during-sidebar-navigation-v110).
- [x] **~~P0~~ — Isolate Azure DevOps state when switching pull requests (shipped in 1.1.0).** ADO can navigate from one pull request to another without replacing the document, leaving the extension's original PR context, API promises, source caches, Changes, Threads, and Outline catalogs alive. The runtime now compares the complete parsed PR identity (origin, organization, project, repository, and PR ID) before route or pending-navigation work. An identity change clears only PR-scoped session entries and performs one automatic document reload so Chromium reinjects the content script with a fresh closure for the new PR; sidebar position, size, tab, and filter preferences remain. A browser regression switches PR IDs and proves old pending/catalog state is removed, PR-B endpoints are used, and only PR-B threads render. A manual in-place reset is deliberately avoided because stale asynchronous completions could repopulate the new PR. Full design and acceptance criteria: [ADO_ADAPTER_PLAN.md → Iteration T](./ADO_ADAPTER_PLAN.md#712-iteration-t--isolate-state-across-pull-request-spa-navigation-v110).
