...
- **"Collapse all" in the Outline tab.** One click folds every section of every Markdown file down to its top-level headings, alongside the existing Fold H1 / H2 / H3 and Expand all controls.

### Changed

- **The comment, reply, and edit forms look like GitHub's own.** New comments open with your avatar and an "Add a comment on line N" header; the Write and Preview tabs share a row with GitHub's formatting icons; the text box uses the regular font with a padded, focus-ringed field; and Comment and Reply stay disabled until you've typed something. The line number stays editable, and the file name moves to the header's tooltip.
- **Changes stand out in rendered Markdown.** Added and removed blocks are highlighted across their full width in the same green and red as GitHub's source diff, with a solid bar down the left edge. Blocks with mixed edits get a solid yellow bar, and the items added or removed inside them are highlighted the same way. Consecutive added or removed blocks, such as a brand-new file, read as one continuous highlight instead of a stack of separate strips.
...
- **The sidebar starts collapsed on every page load.** Each time you open a pull request's Files changed page you see the compact controls in GitHub's toolbar instead of a floating panel. Expand it with the ☰ button or `t` when you need it; it stays open while you work on that page. Pressing `Shift+T` to reset the layout still opens the full sidebar.
